...
function GM:Monitor(control)
	if not IsValid(GAMEMODE.Vars.Monitor) then
		if not control then
		...
		if not nopeinit then
			if game.GetMap() == "freddysnoevent" then
				if not GAMEMODE.Vars.lastcam then GAMEMODE.Vars.lastcam = 7 end
				if not control then LocalPlayer():EmitSound("fnafgm_campop") end
			elseif game.GetMap() == "fnaf2noevents" then
				if not GAMEMODE.Vars.lastcam then GAMEMODE.Vars.lastcam = 12 end
				if not control then LocalPlayer():EmitSound("fnafgm_campop2") end
			elseif game.GetMap() == "fnaf3" then
				if not GAMEMODE.Vars.lastcam then GAMEMODE.Vars.lastcam = 2 end
				if not control then LocalPlayer():EmitSound("fnafgm_campop3") end
			else
				if not GAMEMODE.Vars.lastcam then GAMEMODE.Vars.lastcam = 1 end
				if not control then LocalPlayer():EmitSound("fnafgm_campop") end
			end
		end
		...
		GAMEMODE.Vars.Monitor.Think = function()
			if not control then
				if not LocalPlayer():Alive() or GAMEMODE.Vars.tempostart or (GAMEMODE.Vars.poweroff and GAMEMODE.FT ~= 2) then
				...
				elseif not input.IsKeyDown(KEY_LCONTROL) and lastlstate[1] ~= false then
				...
				if not LocalPlayer():Alive() or LocalPlayer():Team() ~= 2 or GAMEMODE.Vars.gameend then
					GAMEMODE.Vars.Monitor:Close()
				end
				if GAMEMODE.Vars.Monitor.MapS_Anim and GAMEMODE.Vars.Monitor.MapS_Anim:Active() then
					GAMEMODE.Vars.Monitor.MapS_Anim:Run()
				elseif GAMEMODE.Vars.Monitor.MapS_Anim then
		...
		if game.GetMap() == "freddysnoevent" and not control then
			...
			if game.GetMap() == "freddysnoevent" then
				if not control then
					...
					CloseT.OnCursorEntered = function()
						...
						if waitt >= CurTime() then return end
						waitt = CurTime() + 0.5
						GAMEMODE.Vars.Monitor:Close()
						LocalPlayer():EmitSound("fnafgm_camdown")
						if not IsValid(OpenT) then return end
						OpenT:Show()
					end
					...
					GAMEMODE.Vars.Monitor.MapS_Anim = Derma_Anim("maps_anim", GAMEMODE.Vars.Monitor.MapS, function(pnl, anim, delta, data) pnl:SetImageColor(Color(255, 0, 0, 255 * math.abs(delta * 2 - 1))) end)
					GAMEMODE.Vars.Monitor.MapS_Anim:Start(2)
				end
...
function GM:CloseMonitor()
	if IsValid(GAMEMODE.Vars.Monitor) then GAMEMODE.Vars.Monitor:Close() end
end

net.Receive("fnafgmCloseTablet", function(len) GAMEMODE:CloseMonitor() end)
net.Receive("fnafgmSecurityTablet", function(len) GAMEMODE:Monitor() end)
