...
if not sfont and GM.FT == 2 then
	sfont = "OCR A Std"
	sfont2 = "Graph 35+ pix"
	sfont3 = sfont
	sfont4 = sfont2
elseif not sfont and GM.FT == 3 then
	sfont = "5Computers In Love"
	sfont2 = "Graph 35+ pix"
	sfont3 = sfont
	sfont4 = sfont2
elseif not sfont and GM.FT == 4 then
	sfont = "Graph 35+ pix"
	sfont2 = sfont
	sfont3 = "Precious"
	sfont4 = "DJB Get Digital"
elseif not sfont then
	sfont = "Graph 35+ pix"
	sfont2 = sfont
	sfont3 = sfont
	sfont4 = sfont2
...
concommand.Add("fnafgm_menu", function(ply) fnafgmMenu() end)
...
function GM:DrawDeathNotice(x, y)
	if fnafgm_sandbox_enable:GetBool() then BaseClass.DrawDeathNotice(self, x, y) end
end

function GM:HUDWeaponPickedUp(wep)
	if fnafgm_sandbox_enable:GetBool() then BaseClass.HUDWeaponPickedUp(self, wep) end
end

function GM:HUDItemPickedUp(itemname)
	if fnafgm_sandbox_enable:GetBool() then BaseClass.HUDItemPickedUp(self, itemname) end
end

function GM:HUDAmmoPickedUp(itemname, amount)
	if fnafgm_sandbox_enable:GetBool() then BaseClass.HUDAmmoPickedUp(self, itemname, amount) end
end
...
function GM:HUDDrawTargetID()
	...
	if not GAMEMODE.Vars.fnafviewactive and not IsValid(GAMEMODE.Vars.Monitor) and (trace.Entity:IsPlayer() and (LocalPlayer():Team() == trace.Entity:Team() or LocalPlayer():Team() == 2 or LocalPlayer():Team() == TEAM_SPECTATOR)) then
...
net.Receive("fnafgmDS", function(len) GAMEMODE.Vars.DS = tobool(net.ReadBit()) end)
net.Receive("fnafgmCallIntro", function(len) GAMEMODE:CallIntro() end)
net.Receive("fnafgmAnimatronicsList", function(len) GAMEMODE.Vars.Animatronics = net.ReadTable() end)
net.Receive("fnafgmStart", function(len) GAMEMODE.Vars.WillStart = net.ReadFloat() end)
net.Receive("fnafgmAnimatronicsController", function(len) GAMEMODE:Monitor(true) end)
...
function GM:HUDPaint()
	...
	if client:Team() ~= TEAM_UNASSIGNED then
		...
		elseif not GAMEMODE.Vars.tempostart then
			...
			elseif GAMEMODE.FT == 2 and not GAMEMODE.Vars.usingsafezone then
			...
			elseif GAMEMODE.FT ~= 2 and (client:Team() ~= 1 or (not GAMEMODE.Vars.poweroff and client:Alive()) or (not game.SinglePlayer() and not client:Alive() and not GAMEMODE.Vars.poweroff)) then
				...
				if power ~= 0 then
					draw.DrawText((GAMEMODE.TranslatedStrings.powerleft or GAMEMODE.Strings.en.powerleft) .. power .. "%", "FNAFGMNIGHT", 64, powerhs - 64, GAMEMODE.Colors_default, TEXT_ALIGN_LEFT)
				end
			...
			if client:Team() ~= 1 or not client:Alive() and not game.SinglePlayer() then
				local alivec = 0
				for k, v in pairs(player.GetAll()) do
					if v:Alive() and v:Team() == 1 then alivec = alivec + 1 end
				end
	...
	if GAMEMODE.Vars.SGvsA and client:Team() == TEAM_UNASSIGNED and not GAMEMODE.Vars.poweroff then
	...
	if GAMEMODE.Vars.willviewcheck and client:Team() == TEAM_UNASSIGNED then
		...
		elseif game.GetMap() == "fnaf2noevents" and not GAMEMODE.Vars.overfive then
...
hook.Add("PreDrawHalos", "fnafgmHalos", function()
	...
	if client:Team() == 2 then
		for k, v in pairs(player.GetAll()) do
			if v:Team() == 1 and client:Alive() and v:Alive() then table.insert(tab, v) end
		end

		for k, v in pairs(ents.FindByClass("fnafgm_animatronic")) do
			if client:Alive() then table.insert(tab2, v) end
		end
	...
	if client:Team() == TEAM_SPECTATOR then
		for k, v in pairs(player.GetAll()) do
			if v:Team() == 1 and v:Alive() then table.insert(tab, v) end
		end

		for k, v in pairs(player.GetAll()) do
			if v:Team() == 2 and v:Alive() then table.insert(tab2, v) end
		end
...
hook.Add("HUDPaint", "fnafgmInfo", function()
	if GetConVarNumber("cl_drawhud") == 0 then return end
	if not fnafgm_cl_hideversion:GetBool() then
		...
		elseif not GAMEMODE.Vars.AprilFool and not GAMEMODE.Vars.Halloween and not GAMEMODE.Vars.Christmas and GetConVar("fnafgm_forceseasonalevent") ~= nil and GetConVar("fnafgm_forceseasonalevent"):GetInt() == 0 then
...
function GM:RenderScreenspaceEffects()
	...
	elseif client:Team() == 2 and not client:Alive() then
		local colormod = {
			["$pp_colour_addr"] = 0,
			["$pp_colour_addg"] = 0,
			["$pp_colour_addb"] = 0,
	...
	if IsValid(FNaFView) and not IsValid(GAMEMODE.Vars.Monitor) then
...
function fnafgmNotif(str, ne, dur, sound)
	...
	if not tobool(sound) then return end
